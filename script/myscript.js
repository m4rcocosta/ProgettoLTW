...
$(document).ready(function() {
	$("#contenuto").load("htm/home.htm");
	$("#home").click(function() {
		$("#contenuto").load("htm/home.htm");
		return false;
	});
	$("#ubuntu").click(function() {
		$("#contenuto").load("htm/ubuntu.htm");
		return false;
	});
	$("#kubuntu").click(function() {
		$("#contenuto").load("htm/kubuntu.htm");
		return false;
	});
	$("#xubuntu").click(function() {
		$("#contenuto").load("htm/xubuntu.htm");
		return false;
	});
	$("#lubuntu").click(function() {
		$("#contenuto").load("htm/lubuntu.htm");
		return false;
	});
	$("#ubuntuGnome").click(function() {
		$("#contenuto").load("htm/ubuntuGnome.htm");
		return false;
	});
	$("#ubuntuMate").click(function() {
		$("#contenuto").load("htm/ubuntuMate.htm");
		return false;
	});
	$("#edubuntu").click(function() {
		$("#contenuto").load("htm/edubuntu.htm");
		return false;
	});
	$("#ubuntuStudio").click(function() {
		$("#contenuto").load("htm/ubuntuStudio.htm");
		return false;
	});
	$("#mythbuntu").click(function() {
		$("#contenuto").load("htm/mythbuntu.htm");
		return false;
	});
	$("#linuxMint").click(function() {
		$("#contenuto").load("htm/linuxMint.htm");
		return false;
	});
	$("#openSuse").click(function() {
		$("#contenuto").load("htm/openSuse.htm");
		return false;
	});
	$("#fedora").click(function() {
		$("#contenuto").load("htm/fedora.htm");
		return false;
	});
	$("#kaliLinux").click(function() {
		$("#contenuto").load("htm/kaliLinux.htm");
		return false;
	});
	$("#debian").click(function() {
		$("#contenuto").load("htm/debian.htm");
		return false;
	});
	$("#mageia").click(function() {
		$("#contenuto").load("htm/mageia.htm");
		return false;
	});
	$("#archLinux").click(function() {
		$("#contenuto").load("htm/archLinux.htm");
		return false;
	});
});
	...
	(function(){
		...
		var expConIm = function(im){
			...
			if (!im || !expConIm.r.test (im.className))
				return;
			var e = expConIm;		//i prossimi 'e' si riferiscono alla funzione 'expConIm'
			widthHeight = function(dim){		// funzione per altezza e larghezza
				return dim[0] * cos + dim[1] + 'px';
			}
			resize = function(){		//funzione per determinare il ridimensionamento dell'immagine, considerando le variabili 'speed' e 'times' definite sopra
				cos = (1 - Math.cos((e.ims[i].jump / times) * Math.PI)) / 2;
				im.style.width = widthHeight (e.ims[i].w);	//prende la larghezza dell'immagine
				im.style.height = widthHeight (e.ims[i].h);	//prende l'altezza dell'immagine
				if (e.ims[i].d && times > e.ims[i].jump){
					++e.ims[i].jump;
					e.ims[i].timer = setTimeout(resize, speed);		//setta il timeout per il ridimensionamento
				} 
			...
			d = document.images, i = d.length - 1;		//metto in 'd' le immagini nel documento e in 'i' il contatore per il for
			for (i; i > -1; --i)
				if(d[i] == im) break;		//'immagine' è il parametro che passo alla funzione genitore
			i = i + im.src;
			if (!e.ims[i]){			//se non c'è l'immagine
				im.title = '';		//setto il titolo a ''
				e.ims[i] = {im : new Image(), jump : 0};
				e.ims[i].im.onload = function(){	// funzione di ridimensionamento al caricamento dell'immagine
					e.ims[i].w = [e.ims[i].im.width - im.width, im.width];		//larghezza
					e.ims[i].h = [e.ims[i].im.height - im.height, im.height];	//altezza
					e (im);
				}
				e.ims[i].im.src = im.src;
				return;
			}
			if (e.ims[i].timer) clearTimeout(e.ims[i].timer);	//azzera il timeout per il ridimensionamento
			e.ims[i].d = !e.ims[i].d;
			resize ();
		};
...
function inserisciUtente() {
	...
	if (document.registrazione.password1.value.length > 16) {
		alert("La password deve essere lunga al massimo 16 caratteri!");
		return false;
	}	
...
function Email() {
	...
	if (controlloAccesso == 0) {
		alert("Devi effettuare prima l'accesso.\nRegistrati se non l'hai ancora fatto!");
		return false;
	}
